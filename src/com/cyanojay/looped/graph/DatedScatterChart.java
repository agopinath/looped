...
public class DatedScatterChart extends ScatterChart {
	/** The constant to identify this chart type. */
	public static final String TYPE = "DatedScatter";
	...
	public DatedScatterChart(XYMultipleSeriesDataset dataset, XYMultipleSeriesRenderer renderer) {
	...
	protected void drawXLabels(List<Double> xLabels,
		...
		drawXTextLabels(xTextLabelLocations, canvas, paint, true, left, top,
				bottom, xPixelsPerUnit, minX, maxX);
	}
	...
	protected List<Double> getXLabels(double min, double max, int count) {
		...
		if (mStartPoint == null) {
			mStartPoint = min - (min % DAY) + DAY
					+ new Date(Math.round(min)).getTimezoneOffset() * 60 * 1000;
		}
		if (count > 25) {
			count = 25;
		}
